...
class BrainDQNMain(object):
    ...
    def __init__(self, actions):
        ...
        self.Q_net = DeepNetWork().to(device)  # 将模型移到 GPU 或 CPU
        self.Q_netT = DeepNetWork().to(device)  # 将目标网络移到 GPU 或 CPU
        self.load()
    ...
    def train(self):  # Step 1: obtain random minibatch from replay memory
        ...
        nextState_batch = torch.Tensor(nextState_batch).to(device)  # 将下一状态数据移到 GPU

        action_batch = np.array(action_batch)
        index = action_batch.argmax(axis=1)
        print("action " + str(index))
        index = np.reshape(index, [BATCH_SIZE, 1])
        action_batch_tensor = torch.LongTensor(index).to(device)  # 将动作张量移到 GPU

        QValue_batch = self.Q_netT(nextState_batch)
        QValue_batch = QValue_batch.detach().cpu().numpy()  # 确保计算在 CPU 上
        ...
        state_batch_tensor = torch.Tensor(state_batch).to(device)  # 将状态数据移到 GPU
        y_batch_tensor = torch.Tensor(y_batch).to(device)  # 将目标值移到 GPU
    ...
    def getAction(self):
        currentState = torch.Tensor([self.currentState]).to(device)  # 将当前状态移到 GPU
        ...
        if self.timeStep % FRAME_PER_ACTION == 0:
            if random.random() <= self.epsilon:
                ...
                action_index = np.argmax(QValue.detach().cpu().numpy())  # 确保计算在 CPU 上
